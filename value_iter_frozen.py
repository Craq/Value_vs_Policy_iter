...
def evaluatePolicy(env,policy, gamma=1.0,n=100):
	scores = [execute(env,policy,gamma=gamma) for _ in range(n)]
...
def calculatePolicy(v,gamma=1.0):
	...
	for s in range(env.env.nS):
		...
		for a in range(env.action_space.n):
			...
			#tuple (prob,next state, reward, done)
				p,s_,r,_ = next_sr
...
if __name__ == '__main__':
	...
	endTime=time.time()
	print('Best score = %0.2f. Time taken = %4.4f sec'%(np.mean(policy_score),endTime-startTime))